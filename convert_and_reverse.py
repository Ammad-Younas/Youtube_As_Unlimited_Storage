...
class GenerateAndReverse(QWidget):
    ...
    def choose_output_path_for_reverse(self):
        file_dialog = QFileDialog()
        self.output = file_dialog.getExistingDirectory(self, "Choose a folder for output orignal file...")
        if self.output:
    ...
    def start_reverse_video_thread(self):
        if self.choose_file_reverse.text() == '':
            ...
            QMessageBox.warning(self, 'Warning', 'Please choose a path for output orignal file')
        else:
            self.thread = QThread()
...
class ReverseVideoWorker(QObject):
    ...
    def reverse_file(self):
        filename = os.path.join(self.output, 'orignal.mp4')
        width = 1280
        height = 720
        block_size = 4
        ...
        for frame_number in range(1, total_frames):
            frame_image = Image.open(os.path.join(self.output, 'Extracted_Frames', f'frame_{frame_number:09d}.png'))
            frame_data = []
